...
class NBotConversationMain(NBotConversation):
    def __init__(self):
        ...
        self.conversation = ConversationHandler(
            entry_points=[
                CommandHandler('check', self.store_start),
                # CommandHandler('set', self.),
    ...
    def store_start(update: Update, context: NBotContext) -> None:
        ...
        text = f"Choose type or send a new one of the link:\n{context.link_buffer}"
        buttons = [
            [InlineKeyboardButton(text=t, callback_data=t) for t in context.db_container.get_categories()]
        ]
        update.message.reply_text(text=str(text), reply_markup=InlineKeyboardMarkup(buttons))
